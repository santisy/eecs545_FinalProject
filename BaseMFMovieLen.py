...
if __name__ == "__main__":
    ...
    numFolds = 2 # number of folds of data
    testIdx = 1 #index of testing data
    # iterNum = 250 # number of iterations
    log = [] # iteration log
    maxIterNum = 400 # conrolling number of iterations
 
    # data file path, and read
    method = "Basic MF"
    dataset = "MovieLens1M"
    categoryName = 'MovieLens'
    dataPath = "F:\\EECS545Proj\\ml1m\\"    
    ...
    for i in range(1,numFolds + 1):
        dataFID = dataPath + 'test' + str(i) + '.csv'
        if i!= testIdx:
            if flag == 1:
                 trainContent = pd.read_csv(dataFID)
                 # trainContent.columns = dataTerms
                 flag = 0
            else:
                 content = pd.read_csv(dataFID)
                 # content.columns = dataTerms
                 trainContent = pd.concat([trainContent,content])
        else:
            testContent = pd.read_csv(dataFID)
    ...
    itemList = pd.unique(trainContent['movieID']).tolist()
    ...
    for (idx,row) in trainContent.iterrows():
        RUI[userList.index(row['userID']),itemList.index(row['movieID'])] = np.float16(row['rating'])
    ...
    for (idx,row) in testContent.iterrows():
        ...
        try:
            ...
            P_hat = P[itemList.index(row['movieID']),np.newaxis]
