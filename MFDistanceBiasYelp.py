...
# -*- coding: utf-8 -*-
"""
Modifed on Dec 04, with dymatic(distance) bias
Demonstrating PMF can work with prior knowledge
...
if __name__ == "__main__":
    ...
    # Generating distance bias coefficients
    distanceList = np.array(trainContent.distance.tolist())
    distanceList = distanceList/1000 #turn into kilometers 
    ratingList = np.array(trainContent.rating.tolist())
    distanceBiasCoe = np.polyfit(distanceList,ratingList,polyfitDeg)     
    
    # training rating matrix and distance matrix construction
    gc.enable()
    gc.collect()
    RUI = np.zeros([len(userList),len(itemList)], dtype = np.float16)
    DUI = np.zeros([len(userList),len(itemList)], dtype = np.float16)    
    for (idx,row) in trainContent.iterrows():
        RUI[userList.index(row['userID']),itemList.index(row['itemID'])] = np.float16(row['rating'])
        dui = row['distance'] # in kilometers
        if np.isinf(np.float16(dui/1000)):
            print "error! a missing distance detected!"            
        else:
            DUI[userList.index(row['userID']),itemList.index(row['itemID'])] =  np.float16(dui/1000)
    print "RUI and DUI has been constructed!"
    ...
    defaultRatingBias = trainContent['rating'].mean()
    ...
    # took 5 seconds for one teration now. Excited!
        ...
        for j,k in ratingList:
            ...
            if DUI[j][k] <= 0.00:
                bdui = defaultRatingBias = trainContent['rating'].mean()
            else:
                bdui = distanceBias(distanceBiasCoe,float(DUI[j][k]))
            (bu,bi,Uu,Pi,res) = biasedUpdate(BU[j],BI[k],bdui,U[j,np.newaxis],P[k,np.newaxis],RUI[j][k],lambdaU,lambdaP,lambdaBu,lambdaBi)
            U[j] = U[j] - step*Uu
            P[k] = P[k] - step*Pi
            BU[j] = BU[j] - step*bu
            BI[k] = BI[k] - step*bi
            errorNew += res/numRating
            if math.isnan(errorNew): #debugging
                pdb.set_trace()
                print "error!"
        ...
        print "debug: no. of DUI error: ",DUIerror
    ...
    for (idx,row) in testContent.iterrows():
        count += 1            
        testRUI = row['rating']
        testDUI = row['distance']/1000 # get the distance in km
        bdui = distanceBias(distanceBiasCoe,testDUI) # calculate biased ui with distance
        try:
            ...
            RUI_hat = float(np.dot(U_hat,P_hat.T)) + bdui + bu_hat + bi_hat
